...
const assets = [
  {
    ...
    pattern: /src="([^"]*scripts\/main\.js)(?:\?v=[^"]*)?"/g,
  },
  {
    type: "style",
    path: "styles/main.css",
    pattern: /href="([^"]*styles\/main\.css)(?:\?v=[^"]*)?"/g,
  },
  {
    type: "script",
    path: "scripts/title-utils.js",
    pattern: /src="([^"]*scripts\/title-utils\.js)(?:\?v=[^"]*)?"/g,
  },
];
  ...
  assets.forEach((asset) => {
    ...
    content = content.replace(asset.pattern, (match, url) => {
      const newTag = match.replace(
        match,
        `${asset.type === "script" ? "src" : "href"}="${url}?v=${hash}"`,
      );
      if (match !== newTag) {
        hasChanges = true;
      }
